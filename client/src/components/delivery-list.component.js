...
const Delivery = (props) => (
  <tr className='text-lg bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600'>
    ...
    <td className='px-2 py-4'>{props.delivery.cname}</td>
    <td className='px-2 py-4'>{props.delivery.deliveryStatus}</td>
    <td className='px-2 py-4'>{props.delivery.CourierService}</td>
...
export class DeliveryList extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      delivery: [],
      searchDelivery: '',
      courierServiceFilter: '', // Add a filter for Courier Service
  ...
  handleCourierServiceChange = (event) => {
    const selectedCourierService = event.target.value
    this.setState({selectedCourierService}, () => {
      console.log('selected Courier Service', selectedCourierService)
      this.refreshTable()
    })
  }
  refreshTable() {
    axios
      .get('http://localhost:3000/delivery/') // Adjust the API endpoint
      .then((response) => {
        let filteredData = response.data
        if(this.state.selectedCourierService){
          filteredData = filteredData.filter((delivery) =>
          delivery.CourierService
          .toLowerCase()
          .includes(this.state.selectedCourierService.toLowerCase())
          )
        }
        this.setState({ delivery: filteredData }) // Update state variable name
      })
      .catch((error) => {
        console.log(error)
      })
  ...
  deleteDelivery(id) {
    axios
      .delete('http://localhost:3000/delivery/' + id) // Adjust the API endpoint
      .then((res) => {
        console.log(res)
        Swal.fire({
          icon: 'success',
  ...
  searchDeliveryList() {
    return this.state.delivery.map((currentDelivery) => {
      const { searchDelivery, courierServiceFilter } = this.state;
      if (
        this.state.searchDelivery === '' ||
        currentDelivery.orderID.includes(this.state.searchDelivery) ||
        (this.state.courierServiceFilter === '' ||
          currentDelivery.CourierService === this.state.courierServiceFilter)
                ...
                <table>
                  <tr>
                    ...
                    <div className='flex justify-end sm:flex-row sm:text-left sm:justify-end gap-2'>
                        {/* Add the Courier Service filter dropdown */}
                        <select
                          className='form-control rounded-lg text-sm px-5 py-2.5 mr-2 mb-2'
                          value={this.state.selectedCourierService}
                          onChange={this.handleCourierServiceChange}
                        >
                          <option value=''>Select Courier Service</option>
                          {courierServiceOptions.map((option, index) => (
              ...
              <div className='relative grid content-start grid-cols-1 gap-4 overflow-x-auto shadow-md sm:rounded-lg'>
                <table className='w-full h-full overflow-y-auto text-sm text-left text-gray-500 dark:text-gray-400'>
                  <thead className='p-5 text-xs text-gray-700 uppercase border bg-gray-50 dark:bg-gray-700 dark:text-gray-400'>
                    <tr>
                      <th className='p-2 tbhead'>Order ID</th> {/* New Header */}
                      <th className='p-2 tbhead'>Customer Name</th> {/* New Header */}
                      <th className='p-2 tbhead'>Delivery Status</th>{' '}
                      {/* New Header */}
                      <th className='p-2 tbhead'>Courier Service</th>
                      <th className='p-2 tbhead'> Product Details</th> {/* New Header */}
                      <th className='p-2 tbhead'>Total amount</th> {/* New Header */}
