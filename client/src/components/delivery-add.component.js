...
//ihlas
  ...
  onSubmit(e) {
    ...
    }else if (/^\d+$/.test(this.state.cname)) {
      // Use a regular expression to check if the entire string is a number
      this.setState({ cnameError: 'Invalid Customer Name. Name should not be a number.' });
  
  
    } else {
      axios.post('http://localhost:3000/delivery/add', delivery).then((res) => {
        console.log(res)
        if (res.status === 200) {
          this.clearData()
          Swal.fire({
