...
export default class EditDelivery extends Component {
  constructor(props) {
    ...
    this.onChangeTrackingNo = this.onChangeTrackingNo.bind(this)
    //
  ...
  componentDidMount() {
    axios
      .get('http://localhost:3000/delivery/' + this.props.deliveryId)
      .then((response) => {
        this.setState({
          ...
          //ihlas
          CourierService: response.data.CourierService,
  ...
  onChangeCourierService(e) {
    this.setState({ CourierService: e.target.value })
  }
  onChangeTrackingNo(e) {
  ...
  onSubmit(e) {
    ...
    if (this.state.contactNum.length < 10) {
     ...
     //ihlas
    } else if (this.state.CourierService.length <= 2) {
      this.setState({ CourierService: 'Invalid Input.' }) 
  } else if (this.state.TrackingNo.length <= 3) {
      this.setState({ TrackingNo: 'Invalid Tracking Number.' })     
    //  
      ...
      axios
        .put(
          'http://localhost:3000/delivery/' + this.props.deliveryId,
          delivery
        )
        .then((res) => {
          if (res.status === 200) {
                    ...
                    <div className=''>
                      ...
                      <div>
                          ...
                          >
                            ...
                            <option value='SrilankaPost'>Sri Lanka Post</option>
                            ...
                            <p className='block text-lg font-medium text-red-900 dark:text-white'>
                              {this.state.CourierServiceError}
                            </p>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div>
                            <input
                              ...
                              onChange={this.onChangeTrackingNo}
                            />
                            <p className='block text-lg font-medium text-red-900 dark:text-white'>
                              {this.state.TrackingNoError}
                            </p>
                          </div>
                        </div>  
